...
const checkPermission = (permissionCode) => {
    return async (req, res, next) => {
        try {
            const user = req.user;
            console.log(`🔍 [checkPermission] Checking permission: ${permissionCode} for user:`, user?.id);
            ...
            if (!adminRoles || adminRoles.length === 0) {
                const { data: allAdminRoles } = await supabase_1.supabase
                    .from("admin_roles")
                ...
                return res.status(403).json({
                    error: "Bạn chưa được phân quyền. Vui lòng liên hệ quản trị viên để được cấp quyền truy cập.",
                    code: "NO_ROLES_ASSIGNED",
                    message: "Admin account chưa được gán roles. Vui lòng liên hệ quản trị viên.",
                });
            }
            // Lấy tất cả role_ids
            const roleIds = adminRoles.map((ar) => ar.role_id).filter(Boolean);
            // Query role_permissions trực tiếp
            const { data: rolePermsData, error: rolePermsError } = await supabase_1.supabase
            ...
            if (!rolePermsData || rolePermsData.length === 0) {
                return res.status(403).json({
                    error: "Roles của bạn chưa được gán permissions. Vui lòng liên hệ quản trị viên.",
            ...
            // Lấy permission_ids
            const permissionIds = [
                ...new Set(rolePermsData.map((rp) => rp.permission_id).filter(Boolean)),
            ];
            // Query permissions
            const { data: permissionsData, error: permsError } = await supabase_1.supabase
                .from("permissions")
            ...
            if (!permissionsData || permissionsData.length === 0) {
                return res.status(403).json({
                    error: "Không tìm thấy permissions trong hệ thống. Vui lòng liên hệ quản trị viên.",
            ...
            if (userPermissions.has(permissionCode)) {
                console.log(`✅ [checkPermission] Admin ${adminId} có permission: ${permissionCode}`);
                return next();
            }
            // KHÔNG CÓ PERMISSION - Trả về thông báo lỗi rõ ràng
            console.log(`❌ [checkPermission] Admin ${adminId} KHÔNG CÓ permission: ${permissionCode}`);
