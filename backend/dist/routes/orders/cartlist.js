...
exports.registerCartRoutes = registerCartRoutes;
const supabase_1 = require("../../config/supabase");
const authRequired_1 = require("../../middleware/authRequired");
function registerCartRoutes(router) {
    router.get("/cart/customer/:customerId", async (req, res) => {
        const customerId = req.params.customerId;
        try {
            const { data: cart, error: cartError } = await supabase_1.supabase
                .from("cart")
                .select("cart_id")
        ...
        `)
            ...
            if (productIds.length > 0) {
                const { data: productItems, error: itemsError } = await supabase_1.supabase
                    .from("product_item")
                    .select(`
              product_id,
              product_image (
    ...
    // CRUD
    router.post("/cart/add", authRequired_1.authRequired, async (req, res) => {
        ...
        try {
            let { data: cart } = await supabase_1.supabase
                .from("cart")
                .select("cart_id")
            ...
            const { data: productItem } = await supabase_1.supabase
                .from("product_item")
            ...
            const { data: existingItem, error: existingItemError } = await supabase_1.supabase
                .from("cartitems")
                .select("*")
                .eq("cart_id", cart.cart_id)
                .eq("product_item_id", product_item_id)
            ...
            if (existingItem) {
                ...
                const { data: dataUpdate, error } = await supabase_1.supabase
                    .from("cartitems")
                    .update({
                ...
                const { data: newItem, error } = await supabase_1.supabase
                    .from("cartitems")
                    .insert({
    ...
    router.put("/cart/update", authRequired_1.authRequired, async (req, res) => {
        const customer_id = req.user?.id;
        const { product_item_id, quantity } = req.body;
        if (!customer_id) {
            return res.status(401).json({ error: "Chưa đăng nhập" });
        ...
        try {
            const { data: cart } = await supabase_1.supabase
                ...
                .single();
            if (!cart)
                throw new Error("Không thấy thông tin sản phẩm");
            const { data: productItem } = await supabase_1.supabase
                .from("product_item")
    ...
    router.delete("/cart/delete", authRequired_1.authRequired, async (req, res) => {
        const customer_id = req.user?.id;
