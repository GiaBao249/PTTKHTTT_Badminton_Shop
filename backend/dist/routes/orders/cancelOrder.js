...
exports.registerCancelOrderRoutes = registerCancelOrderRoutes;
const supabase_1 = require("../../config/supabase");
const authRequired_1 = require("../../middleware/authRequired");
function registerCancelOrderRoutes(router) {
    router.patch("/cancel/:orderId", authRequired_1.authRequired, async (req, res) => {
        ...
        try {
            ...
            const { data: orderDetails, error: detailError } = await supabase_1.supabase
                .from("orderdetail")
            ...
            if (orderDetails && orderDetails.length > 0) {
                for (const detail of orderDetails) {
                    const { data: productItem, error: getItemError } = await supabase_1.supabase
                        .from("product_item")
                        .select("quantity")
                        .eq("product_item_id", detail.product_item_id)
                        .single();
                    if (getItemError) {
                        console.error(`Error getting product_item ${detail.product_item_id}:`, getItemError);
                        continue;
                    }
                    if (productItem) {
                        ...
                        const { error: updateError } = await supabase_1.supabase
                            ...
                            .eq("product_item_id", detail.product_item_id);
                        if (updateError) {
                            console.error(`Error updating product_item ${detail.product_item_id}:`, updateError);
                        }
                        else {
            ...
            const { data: updatedOrder, error: updateOrderError } = await supabase_1.supabase
                .from("orders")
