...
exports.registerTopByCategoryRoutes = registerTopByCategoryRoutes;
const supabase_1 = require("../../config/supabase");
function registerTopByCategoryRoutes(router) {
    router.get("/top-by-categories", async (req, res) => {
        try {
        ...
        `)
            ...
            if (productList.length === 0)
                return res.json(categoryIds.map((id) => ({
                    category_id: id,
                    category_name: "",
                    products: [],
                })));
        ...
        `)
            ...
            if (productItemList.length === 0) {
                const result = categoryIds.map((catId) => {
                    const categoryProducts = productList.filter((p) => p.category_id === catId);
                    let categoryName = "";
            ...
            const productItemIds = productItemList.map((it) => it.product_item_id);
            const productIdToInventory = new Map();
            productItemList.forEach((it) => {
            ...
            const { data: orderDetails, error: odError } = await supabase_1.supabase
                .from("orderdetail")
