...
exports.registerCheckoutRoutes = registerCheckoutRoutes;
const supabase_1 = require("../../config/supabase");
const authRequired_1 = require("../../middleware/authRequired");
function registerCheckoutRoutes(router) {
    router.post("/checkout", authRequired_1.authRequired, async (req, res) => {
        const user = req.user;
        ...
        try {
            ...
            const { data: productItems, error } = await supabase_1.supabase
                .from("product_item")
                .select("*")
            ...
            for (const item of cart_items) {
                const productItem = productItems?.find((p) => item.product_item_id === p.product_item_id);
            ...
            if (!finalAddressId && req.body.shipping_info) {
                const { data: newAddress, error: newAddressError } = await supabase_1.supabase
                    .from("address")
                    .insert({
            ...
            const orderDetails = cart_items.map((item) => ({
                order_id: order.order_id,
                product_item_id: item.product_item_id,
                quantity: item.quantity,
                amount: item.total_amount || item.quantity * item.price,
            }));
            const { error: detailError } = await supabase_1.supabase
                .from("orderdetail")
                .insert(orderDetails);
            if (detailError)
                throw detailError;
            ...
            if (payment_method === "cod") {
                ...
                if (cart && !cartError && cart.cart_id) {
                    const checkedOutProductIds = cart_items.map((item) => item.product_item_id);
                    const { error: deleteCartError } = await supabase_1.supabase
                        .from("cartitems")
                        .delete()
                        .eq("cart_id", cart.cart_id)
                        .in("product_item_id", checkedOutProductIds);
                    if (deleteCartError) {
                        console.error("Error deleting checked out cart items:", deleteCartError);
                    }
                }
                return res.json({
                    success: true,
