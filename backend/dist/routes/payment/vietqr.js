...
function registerVietQRRoutes(router) {
    ...
    router.post("/create-qr", authRequired_1.authRequired, async (req, res) => {
        try {
            ...
            let qrUrl;
            try {
                qrUrl = (0, vietqr_1.createVietQRCode)(amount, order_info || `Thanh toan don hang ${order_id}`);
            }
            catch (error) {
            ...
            const qrData = (0, vietqr_1.createVietQRData)(amount, order_info || `Thanh toan don hang ${order_id}`);
            const response = {
                success: true,
                qr_url: qrUrl,
                qr_data: qrData,
                order_id: order_id,
                amount: amount,
                bank_name: (0, vietqr_1.getBankName)(vietqr_1.vietqrConfig.bankCode),
                account_number: vietqr_1.vietqrConfig.accountNumber,
                account_name: vietqr_1.vietqrConfig.accountName,
                message: `Vui lòng quét mã QR và chuyển khoản ${amount.toLocaleString("vi-VN")} VND`,
            };
     ...
     */
    router.post("/check-payment", authRequired_1.authRequired, async (req, res) => {
        try {
            ...
            return res.json({
                ...
                message: "Vui lòng kiểm tra trạng thái thanh toán trong tài khoản ngân hàng",
            });
        }
        catch (error) {
     ...
     */
    router.post("/confirm-payment", authRequired_1.authRequired, async (req, res) => {
        try {
            ...
            const { data: orderDetails, error: detailsError } = await supabase_1.supabase
                .from("orderdetail")
                .select("product_item_id, quantity")
                .eq("order_id", order_id);
            if (!detailsError && orderDetails) {
                for (const detail of orderDetails) {
                    const { data: productItem, error: itemError } = await supabase_1.supabase
                        .from("product_item")
                        .select("quantity")
                ...
                if (cart && cart.cart_id) {
                    const checkedOutProductIds = orderDetails.map((item) => item.product_item_id);
                    await supabase_1.supabase
                        .from("cartitems")
                        .delete()
    ...
    router.post("/test-payment", async (req, res) => {
        try {
            ...
            const { data: orderCheck, error: checkError } = await supabase_1.supabase
                .from("orders")
            ...
            return res.json({
                ...
                order_id: order_id,
                status: "Processing",
                message: "Test payment thành công - Đơn hàng đã được xác nhận (TEST MODE)",
                test_mode: true,
            });
        }
        catch (error) {
